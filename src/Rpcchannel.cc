                             ...
                             ::google::protobuf::Closure *done)
{
    ...
    krpcheader.set_server_name(service_name);
...
bool KrpcChannel::newConnect(const char *ip, uint16_t port)
{
    ...
    if (-1 == connect(clientfd, (struct sockaddr *)&server_addr, sizeof(server_addr)))
    {
        ...
        std::cout<< "connect server error"<<errtxt;
        return false;
    }
    m_clientfd = clientfd;
    return true;
...
std::string KrpcChannel::QueryServiceHost(Zkclient *zkclient,std::string service_name,std::string method_name, int& idx){
    ...
    if (host_data_1 == "")
    {
        std::cout<< method_path + " is not exist!";
        return" ";
    }
    idx = host_data_1.find(":"); // 127.0.0.1:8000 获取到ip和port的分割符
    if (idx == -1)
    {
        std::cout<< method_path + " address is invalid!";
        return" ";
    }
    return host_data_1;
}
KrpcChannel::KrpcChannel(std::string ip, uint16_t port, bool connectNow) : m_clientfd(-1), zk_ip(ip), zk_port(port), m_idx(0)
{
    if(!connectNow){
        return ;
    }
