...
void* TcpConnectionPool::allocate() {
    std::unique_lock<std::mutex> lock(mutex_);
    if (!pool_.empty()) {
        void* memory = pool_.front();
        pool_.pop();
        return memory;
    }
    // 如果池中没有可用内存，则分配新的内存
    return ::operator new(sizeof(TcpConnection));
}

// 归还一个 TcpConnection 对象的内存
void TcpConnectionPool::deallocate(void* memory) {
    std::unique_lock<std::mutex> lock(mutex_);
    if (pool_.size() < max_pool_size_) {
        pool_.push(memory); // 归还到内存池
    } else {
        ::operator delete(memory); // 真正释放内存
    }
}
...
void* TcpConnection::operator new(std::size_t size) {
    // if (size != sizeof(TcpConnection)) {
    //     return ::operator new(size);
    // }
    return memoryPool::HashBucket::useMemory(size);
}

// 重载 operator delete
void TcpConnection::operator delete(void* memory) {
    memoryPool::HashBucket::freeMemory(memory,sizeof(TcpConnection));
    // TcpConnectionPool::getInstance().deallocate(memory);
}